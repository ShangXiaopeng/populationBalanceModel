...
:
	...
	moment1_(readScalar(dict_.lookup("moment1"))),
	moment2_(readScalar(dict_.lookup("moment2")))
...
const Foam::dimensionedScalar
Foam::diameterModels::populationBalanceModel::gamma2
(
    const label i,
    const dimensionedScalar& v
) const
{
    ...
    else if (v.value() <= xi.value())
    {
        ...
        return (pow(v, moment2_)*pow(lowerBoundary, moment1_) - pow(v, moment1_)*pow(lowerBoundary, moment2_))
		/(pow(xi, moment2_)*pow(lowerBoundary, moment1_) - pow(xi, moment1_)*pow(lowerBoundary, moment2_));
    }
    else
    {
        return (pow(v, moment2_)*pow(upperBoundary, moment1_) - pow(v, moment1_)*pow(upperBoundary, moment2_))
		/(pow(xi, moment2_)*pow(upperBoundary, moment1_) - pow(xi, moment1_)*pow(upperBoundary, moment2_));
...
const Foam::scalar& Foam::diameterModels::populationBalanceModel::Moment1() const
{
	return moment1_;
}
	
const Foam::scalar& Foam::diameterModels::populationBalanceModel::Moment2() const
{
	return moment2_;
}

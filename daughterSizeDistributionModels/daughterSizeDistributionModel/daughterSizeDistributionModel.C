...
:
    breakup_(breakup)
//	nodes_({0.9061798459, -0.9061798459, 0.5384693101, -0.5384693101, 0.}),
//	A_({0.2369268851, 0.2369268851, 0.4786286705, 0.4786286705, 0.5688888889})
	
//	moment1_(readScalar(dict.lookup("moment1"))),
//	moment2_(readScalar(dict.lookup("moment2")))
{
...
//	nodes_[5] = {0.9061798459, -0.9061798459, 0.5384693101, -0.5384693101, 0.};
//	A_[5] = {0.2369268851, 0.2369268851, 0.4786286705, 0.4786286705, 0.5688888889};
}
...
void Foam::diameterModels::daughterSizeDistributionModel::correct()
{
    if (nik_.size() == 0)
    {
	    ...
	    nik_.append(new PtrList<dimensionedScalar>());
            ...
            for (label i = 0; i <= k; i++)
            {
                nik_[k].append(new dimensionedScalar (this->calcNik2(i, k)));
	    }
        }
    }
...
Foam::diameterModels::daughterSizeDistributionModel::calcNik2
(
	const label i,
    const label k
) const
